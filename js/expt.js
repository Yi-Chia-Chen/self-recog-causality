...
class trialObject {
    ...
    run() {
        ...
        this.progress = Math.round( 100 * (this.trialNum+this.pracTrialN) / NOW_TOTAL_TRIAL_N );
        this.trialNum++;

        const FORMAL = this.trialNum > 0;
        if (FORMAL) this.pastFormalTrialN++;

        const LAST = FORMAL ? this.trialNum == NOW_FORMAL_TRIAL_N : this.trialNum == 0;
    ...
    checkForRest() {
        if (this.restTrialN) return this.restTrialN > 0 && trial.trialNum > 0 && this.trialNum % this.restTrialN == 0;
        else return false;
    }
...
class instrObject {
    constructor(options = {}) {
        ...
        this.index = 0;
        this.quizAttemptN = {};
        for (let i=0;i<this.quizConditions.length;i++){
