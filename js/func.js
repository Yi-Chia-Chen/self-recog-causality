...
function REPLACE_SLASH_R_N_WITH_BR(str) {
    return str.replace(/(?:\r\n|\r|\n)/g, '<br />')
}

function REPLACE_LINE_BREAK_IN_ENTRY_VALUE(pair) {
    return [pair[0], REPLACE_SLASH_R_N_WITH_BR(pair[1])]
}
...
function FACTORIAL_COND(factor_list) {
    ...
    let all_conditions = [];
    for (let i = 0; i < now_factor.length; i++) {
...
function CHECK_IF_ALL_OBJECT_KEYS_HAVE_DEFINED_VALUE(open_ended_list, choice_list) {
    const OPEN_ITEMS = Object.entries(open_ended_list);
    const CHOICE_ITEMS = Object.entries(choice_list);

    let no_response_n = OPEN_ITEMS.filter(([key, value]) => value.replace('<br />','') == '').length;
    no_response_n += CHOICE_ITEMS.filter(([key, value]) => typeof value == 'undefined').length;
...
function CHECK_IF_FULLSCREEN() {
    if (
        ...
        document.mozFullScreenElement || /* Firefox 9-46 */
