...
class gameObject {
    ...
    update(this_game, show_end_object) {
        this.reset_parameters();
        this.setup_trial_condition(this_game);
        this.showTask(show_end_object);
        this.mark.show();
    ...
    setup_trial_condition(this_game) {
        ...
        this.actualMarkCenterPos = this.rotatePos(this.orientation, {
            x: this.markCenterX,
            y: this.markCenterY
        });
        this.otherlastPos = {
        ...
        const OBJECT_CENTER_POS = this.rotatePos(this.orientation, {
            x: this.objectInitialCenterX,
            y: this.objectInitialCenterY
        });
        ...
        this.objectActualXSpeed = Math.abs(
            this.objectSpeed * Math.cos(TO_RADIANS(this.orientation))
        );
        this.objectActualYSpeed = Math.abs(
            this.objectSpeed * Math.sin(TO_RADIANS(this.orientation))
        );
        this.objectActualXDistToBorder = Math.min(
            Math.abs(OBJECT_X),
            this.width - this.objectWidth - Math.abs(OBJECT_X)
        );
        this.objectActualYDistToBorder = Math.min(
            Math.abs(OBJECT_Y),
            this.height - this.objectHeight - Math.abs(OBJECT_Y)
        );
        this.staticEndTime = Math.min(
            this.objectActualXDistToBorder / this.objectActualXSpeed,
            this.objectActualYDistToBorder / this.objectActualYSpeed
        );
        this.actualDestination = this.rotatePos(this.orientation, {
            x: this.destinationX,
            y: this.destinationY
        });
    ...
    start(game_start_time) {
        ...
        const STANDARDIZED_START_POS = this.unrotatePos(this.orientation, {
            x: this.startPos.x,
            y: this.startPos.y
        });
    ...
    checkToTrigger() {
        if (!this.triggered) {
            ...
            if (this.triggered) {
                ...
                if (this.tiggerType !== "static") {
                    this.startToTriggerDuration =
                        (this.triggerTime - this.startTime) / 1000;
                }
            }
        }
    ...
    checkToContinue() {
        ...
        if (this.objectEnd && this.traceEnd && this.tempo_practice) {
            ...
            this.wipeTask();
            this.startToObjectEndRT = (Date.now() - this.startTime) / 1000;
            $("body").css("cursor", "default");
            this.gameEndCallback();
        } else if (this.objectEnd && this.traceEnd) {
            this.startToObjectEndRT = (Date.now() - this.startTime) / 1000;
            this.recognitionStart();
        } else {
            this.standardizedPosForCheck = this.unrotatePos(
                this.orientation,
                this.posForCheck
            );
            this.deviated = this.checkDeviation();
            if (this.deviated && !this.tempo_practice) {
                this.soundElement.pause();
                this.traceRT = (Date.now() - this.startTime) / 1000;
                this.endDeviationX = this.lastPos.x - this.actualDestination.x;
                this.endDeviationY = this.lastPos.y - this.actualDestination.y;
    ...
    updateDistToDestination() {
        this.distToDestination = DISTANCE_BETWEEN_POINTS(
            [this.posForCheck.x, this.posForCheck.y],
            [this.actualDestination.x, this.actualDestination.y]
        );
    }
    ...
    objectMove() {
        ...
        this.actualObjectPos = this.rotatePos(this.orientation, {
            x: this.objectX,
            y: this.objectY
        });
        ...
        const OBJECT_STAYED_LONG_ENOUGH =
            this.triggerType == "static" &&
            this.elapsedTime >= this.staticEndTime;
        if (OBJECT_REACHED || OBJECT_STAYED_LONG_ENOUGH) {
            this.objectEnd = true;
            if (this.triggerType != "static") {
            ...
            this.objectX =
                this.objectInitialX + this.objectSpeed * this.elapsedTime;
            this.objectCenterX = this.objectX + this.objectWidth / 2;
            const OBJECT_CENTER_POS = this.rotatePos(this.orientation, {
                x: this.objectCenterX,
                y: this.objectInitialCenterY
            });
            const OBJECT_X = OBJECT_CENTER_POS["x"] - this.objectWidth / 2;
            const OBJECT_Y = OBJECT_CENTER_POS["y"] - this.objectHeight / 2;
    ...
    updateDeviationX() {
        ...
        const CORRECT_PROGRESS =
            BEAT_COUNT_AFTER_FIRST / (this.peakValleyN - 1);
        const STANDARDIZED_CORRECT_X =
            CORRECT_PROGRESS * this.sinewaveWaveWidth + this.xPadding;
        this.xDeviation = Math.abs(
            this.standardizedPosForCheck.x - STANDARDIZED_CORRECT_X
        );
    }
    ...
    updateDeviationY() {
        const STANDARDIZED_CORRECT_Y = SINEWAVE_FUNCTION(
            this.standardizedPosForCheck.x
        );
        this.yDeviation = Math.abs(
            this.standardizedPosForCheck.y - STANDARDIZED_CORRECT_Y
        );
    }
    ...
    recognitionStart() {
        ...
        $(document).keyup(function (e) {
            if (e.code == "KeyY" || e.code == "KeyN") {
                ...
                that.recognitionRT =
                    (Date.now() - that.recognitionStartTime) / 1000;
